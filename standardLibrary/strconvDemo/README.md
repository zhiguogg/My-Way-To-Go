...
fmt.Printf("type is %T,value is %v\n",a,a) //type is bool,value is false
```
...
fmt.Printf("type is %T,value is %v\n",b,b)  //type is int64,value is -123
```
...
`ParseUint`类似`ParseInt`但**不接受正负号**，用于无符号整型。
...
fmt.Printf("type is %T,value is %v\n",c,c) //type is int64,value is 123
```
...
如果s合乎语法规则，函数会返回**最为接近s**表示值的一个浮点数（使用IEEE754规范舍入）。
...
fmt.Printf("type is %T,value is %v\n",d,d) //type is float64,value is 3.1415
```
